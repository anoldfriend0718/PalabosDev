...
int main(int argc, char *argv[]) {
  ...
  const string outputDir = "./tmp_lyz_guo/";
  ...
  MultiBlockLattice2D<T, DESCRIPTOR> lattice(
      ...
      new Porosity_BGKdynamics<T, DESCRIPTOR>(parameters.getOmega(),1));
	...
	for (int i = (-20+parameters.getNx()/2); i < (20+parameters.getNx()/2); ++i)
	{
		for (int j = (-20+parameters.getNy()/2); j < (20+parameters.getNy()/2); ++j)
		{
			if (pow(i-50,2)+pow(j-50,2)<=400)
			    lattice.get(i,j).getDynamics().setParameter(dynamicParams::porosity,1);
			else
			    lattice.get(i,j).getDynamics().setParameter(dynamicParams::porosity,0.1);
		}
	}
  */
